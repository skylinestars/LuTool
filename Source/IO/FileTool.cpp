...
bool LuTool::FileTool::CreateFolderRecursive(const std::string &folderName)
{
    ...
    {
        // 将路径标准化
        std::string temp = folderName;
        StrTool::NormalPath(temp);
        // 获取父级目录
        std::string parentFolder = temp.substr(0, temp.find_last_of('/'));
        // 递归创建父级目录
        if (CreateFolderRecursive(parentFolder))
        {
            return CreateFileFolder(temp);
        }
        else
        {
            return false;
